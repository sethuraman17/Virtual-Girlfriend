...
This project has been extended to include two primary modes: `/interview` and `/demo`.

### Interview Mode (`/interview`)

The interview mode is designed to simulate a job interview. The user can upload their resume, which is then summarized and displayed on the screen. The avatar will then ask questions based on the user's resume, providing a realistic interview experience.

### Demo Mode (`/demo`)

The demo mode showcases the application's malpractice detection capabilities. It uses a webcam and a machine learning model to detect the following:

*   **Multiple People:** The system will issue a warning if more than one person is detected in the frame.
*   **Mobile Phone Usage:** The system will issue a warning if a mobile phone is detected in the frame.
...
Open [http://localhost:5173/](http://localhost:5173/) with your browser to see the result. By default, you will be in the `/interview` mode. To access the demo mode, navigate to `http://localhost:5173/demo`.
